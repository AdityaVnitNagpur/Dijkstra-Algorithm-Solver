...
blocks.addEventListener("click", (e) => {
  if (addEdge) return;
  if (cnt > 12) {
    alert("You can add maximum 12 nodes");
    return;
  }
  console.log(e.x, e.y);
  appendBlock(e.x, e.y);
...
const drawLine = (x1, y1, x2, y2, ar) => {
  // prevent multiple edges for same 2 nodes
  ...
  //Rotates the edges to align with the nodes.
  line.style.transform = `rotate(${
    x1 > x2 ? Math.PI + Math.atan(slope) : Math.atan(slope)
  }rad)`;

  //Rotates the edge weight to remain readable.
...
const drawUsingId = (ar) => {
  if (ar[0] === ar[1]) {
    //Resets the background color of the selected node to the default
    document.getElementById(arr[0]).style.backgroundColor = "#333";
    //Resets the array to clear the selection
    arr = [];
    return;
  }
...
//////////////////////// Function to find shortest path from given source node to all other nodes/////////////////////////////////
const findShortestPath = (el) => {
  ...
  if (source >= cnt || isNaN(source)) {
    alert("Enter a valid source node");
    return;
  }
  ...
  while (unvisited.length) {
    // Finds the index of the node with the minimum cost
    let mini = cost.indexOf(Math.min(...cost));
    //  Adds the node to the visited array
    visited.push(mini);
    // Removes the node from the unvisited array
  ...
  // Calls a function to visually indicate the shortest path on the screen
...
const printPath = async (parent, j, el_p) => {
  ...
  // Recursive call for the parent of the current(jth) node
...
const resetDrawingArea = () => {
  ...
  p.innerHTML = "Click to create a node";
